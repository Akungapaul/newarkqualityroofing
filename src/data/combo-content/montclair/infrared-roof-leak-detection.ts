...
export const montclairInfraredRoofLeakDetection: ComboContent = {
  ...
  faqs: [
    {
      ...
      answer: 'The technology is identical -- both use infrared cameras to detect temperature differentials on the roof surface. The difference is scope and purpose. Thermal imaging inspection is a comprehensive survey that maps moisture conditions across the entire roof for asset management planning. Infrared leak detection is targeted diagnostic work focused on locating the specific entry point of an active leak reported by a building occupant. Leak detection typically involves a focused scan of the area above the interior damage, correlated with membrane surface inspection and core sampling, to produce a precise repair recommendation for the specific leak problem.'
    },
    {
      question: 'Can infrared detection find leaks on any type of commercial roof?',
      answer: 'Infrared detection works on membrane roofs (EPDM, TPO, PVC, modified bitumen) over insulation board assemblies where the insulation retains moisture and creates detectable thermal differential. The method is less effective on ballasted roofs where gravel or pavers obscure the membrane thermal signal, and on uninsulated metal deck roofs where no insulation layer exists to retain moisture. For ballasted roofs, we remove ballast in targeted areas to expose the membrane for scanning. For metal deck roofs without insulation, alternative diagnostic methods such as electronic leak detection provide better results.'
    },
    {
      question: 'How much does infrared leak detection cost for Montclair buildings?',
      answer: 'Targeted infrared leak detection for a specific active leak on a Montclair commercial building typically costs six hundred to twelve hundred dollars including the scan, core sampling at identified anomaly locations, and a diagnostic report with repair recommendations. This investment is typically recovered within the first repair cycle by eliminating incorrect repair attempts that would otherwise consume the same budget without resolving the leak. For buildings with multiple active leaks or comprehensive survey needs, costs scale with roof area and complexity.'
    },
    {
